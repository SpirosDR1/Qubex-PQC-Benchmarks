...
def monitor_arbitrum_exposure():
    ...
    print(f"Target: Arbitrum Mainnet | RPC: {ARBITRUM_RPC}\n")
        ...
        try:
            ...
            if current_block.number > last_block.number:
                ...
                if exposure_window > 0:
                    print(f"{Fore.RED}[CRITICAL FUD ALERT] ARBITRUM MAINNET TVL EXPOSED!{Style.RESET_ALL}")
                    print(f"{Fore.RED}-> ECDSA Exposure Window: {exposure_window}ms{Style.RESET_ALL}")
...
if __name__ == "__main__":
    if w3.is_connected():
        monitor_arbitrum_exposure()
    else:
        print("Failed to connect to Arbitrum RPC. Check endpoint.")
